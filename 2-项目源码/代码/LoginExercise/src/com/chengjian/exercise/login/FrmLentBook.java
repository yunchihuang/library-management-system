...
public class FrmLentBook extends JFrame implements ActionListener {

	private JPanel Panel,buttonPanel,Panel2,scorePanel;
	private JScrollPane scoreScrPanel;
	private JLabel lblQuery,lblName,lblnum;
	private JTextField txtQuery,txtName,txtnum;
	private JButton btnEnsure,btnLent;
...
//		this.buttonPanel.setBounds(new Rectangle(0,140,600,80));
		...
		this.lblQuery.setBounds(new Rectangle(20, 20, 150,30));
		this.Panel.add(this.lblQuery);
		
		String []str=new String[]{"id","作者","书名"};
		this.combox=new JComboBox(str);
		this.combox.setBounds(new Rectangle(140,20,60,30));
		this.Panel.add(this.combox);
		
		this.txtQuery=new JTextField();
		this.txtQuery.setBounds(new Rectangle(240,20,150,30));
		this.Panel.add(this.txtQuery);
		
		this.btnEnsure=new JButton("查询");
		this.btnEnsure.setBounds(new Rectangle(415,20,80,30));
		this.btnEnsure.addActionListener(this);
		this.Panel.add(this.btnEnsure);
		
		this.lblName=new JLabel("请输入借阅者姓名：");
		this.lblName.setBounds(new Rectangle(20, 70, 150,30));
		this.Panel.add(this.lblName);
		
		this.txtName=new JTextField();
		this.txtName.setBounds(new Rectangle(140,70,130,30));
		this.Panel.add(this.txtName);
		
		this.lblnum=new JLabel("请输入借阅数量：");
		this.lblnum.setBounds(new Rectangle(280, 70, 150,30));
		this.Panel.add(this.lblnum);
		
		this.txtnum=new JTextField();
		this.txtnum.setBounds(new Rectangle(390,70,130,30));
		this.Panel.add(this.txtnum);
...
public static void main(String[] args) {
	FrmLentBook frm=new FrmLentBook();
	frm.setVisible(true);
}
}
